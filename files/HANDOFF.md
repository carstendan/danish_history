...
| `c01_body.html` … `c45_body.html` | see below. Forty-five retained bodies; the `c16a`/`c16b` and `e12`–`e15` names are retired. **01–24 are authored: edit the body. 25–45 are build inputs regenerated by `mkbody.py` from the drafts — a part build never reads a draft (item 138).** 01–15 were recovered from the pages in item 141 |
...
3. ~~**`build_part_d.py` reads `e12_body.html`–`e15_body.html`**~~ **CLOSED, item
   141**: the recovered bodies are `c12`–`c15` and the config says so. Original entry:
   on the old `e`
...
4. ~~**`build_parts_abc.py` and `build_part_d.py` cannot currently rebuild.**~~
   **CLOSED, item 141**: both use `--band` and refuse a missing token. Original entry: Both do
...
141. **The consistency review, session 3: chapters 01–15 unblocked, the A–D
   findings fixed, the Part I items closed, Part A read. Every chapter in the book
   now has a source in `files/`.** 19 September 2026. Full record in
   `REVIEW-CONSISTENCY.md` §7.

   **Cold run on a fresh clone of `6aed653`: every figure matched
   START_HERE_review_3.** Four commits, each rebuilt and run through the whole suite
   before the next.

   **1 — THE RECOVERY, alone, changing no prose.** `debuild.py extract` → fifteen
   bodies, `c01`–`c15_body.html`, now authored source. Part D's figures written out:
   **eleven** `svg_*.txt`, not ten — page 14 carries a herring-market figure the
   config did not know. **`build_parts_abc.py` was not missing**: it existed, matched
   every page, and could not run for open item 4's reason, as `build_part_d.py`
   could not. Both use `--band` now and refuse a missing token; both refuse a config
   whose section ids disagree with the body — chapter 14's config had an eleventh
   section the page never had. **Chapter 12's page carried a checkpoint correction
   the script never had** (artifact-only, the fifth); the page's wording is kept.
   Rebuilt pages differ from the shipped ones only in stylesheet and minutes stamp.
   **debuild 45 identical**, and the commit reproduces byte for byte from a fresh
   clone. Open items 3 and 4 close.

   **2 — THE FIFTEEN A–D FINDINGS**, all fixed, plus §4.3's D-1 breaks, each read
   against its target. Beyond the list: **07's Golden Horns arrow was insolvent at
   any number** (32 never mentions the horns); **07 and 08 called 1864 "chapter 33"**
   in Thread notes the sweep cannot see; **pages 18–20 said "17 ·", "18 ·", "19 ·" in
   `<title>`** since August — `sweep_arrows.py` has a check 9b for it now.

   **3 — THE PART I ITEMS, closed.** **Sønderborg is two votes apart, not ten**:
   1,680 to 1,682. The 1,672 came from the local archive's booklet, which misprints
   its own Danish total (2,021; its table sums to 2,029). ***Statistiske
   Efterretninger* 1920 nr. 23 is not the plebiscite** — item 104 had found that;
   38's Sources and START_HERE had not caught up. Tønder is two returns and the
   prose now says so. 38 §04's Ribe clause follows chapter 19. Three of 42's open
   questions are in 43's Sources.

   **4 — PART A READ.** Twenty-six errors of fact fixed (§7.4) — among them 02's
   "an eighth of the span" (a third), 01's Hamburg culture "named for the Ahrensburg
   valley", 01's Slotseng-to-Ahrensburg "four thousand years" (two and a half),
   03's copper arriving "at the very end of the Funnel Beaker period" (it came with
   the first farmers, and imports stopped c. 3,300–2,350), 03's Indo-European as
   the ancestor of "the language this page is *not* written in", 02's Kitchen Midden
   Commission "appointed by the government" (the Academy, 1848). Ten repetitions,
   seams and slips fixed (§7.5), each named. D-15: nothing to change in Part A. D-13:
   no case.

   **CHECKED BY A SEPARATE AGENT** that had not seen the work: it confirmed the
   facts and found eleven slips, ten of them in my own edits. All corrected but
   one: **01's Figure 3 still draws the Younger Dryas 150 years after Laacher See,
   where the prose now says two hundred**. It is an inline SVG with no generator,
   and typing coordinates is what "compute, never type" forbids (§7.6).

   **VERIFIED:** build A–E and I, `mkbody` for 38, 42, 43 (no `!!`), `linkindex`,
   `index_generator`, then the whole suite: **debuild 45 identical; 45 of 45,
   336,699 page words, 26.7 h; vignettes 114/84, selftest passes; figcheck 98/30/0;
   draftnotes clean; appcheck 159; freshcheck 14; seams and fixture pass; sweeps:
   ...
   **LESSON.** START_HERE said to *write* A–C build scripts, and one had existed
   ...
   already found it was the wrong publication. Both times the answer was in the
